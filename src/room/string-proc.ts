...
export class StringProc {
  ...
  static transformWayto (from : ValidRoom, userland = false) {
  ...
  static transformTimeto (from : ValidRoom, userland = false) {
  ...
  static transform (from: ValidRoom, userland = false) {
    return [...this.transformWayto(from, userland), ...this.transformTimeto(from, userland)]
  }
  ...
  constructor (readonly kind: Kind, readonly source : SerializedProc, readonly from : string, readonly to : string) {
    ...
    // Generate userland format paths for cartographer
    this.userlandPath = `${this.kind}/stringproc-${this.to}.rb`
